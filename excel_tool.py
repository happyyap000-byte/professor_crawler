...
def search_grade(professor_name):
    """
    搜尋所有 grades_*.csv 檔案，合併資料，並根據教授姓名查找 A+ 比例。
    """

    # 1. 資料讀取與合併
    try:
        # 找出所有符合 'grades_*.csv' 規則的檔案
        all_files = glob.glob("grades_*.csv")

        if not all_files:
            return ">> 錯誤：找不到任何 grades_*.csv 檔案。請確認它們在專案根目錄下。"
        ...
        for f in all_files:
            try:
                ...
                return (
                    f"❌ 讀取檔案 {f} 失敗：欄位名稱錯誤。\n"
                    f"請確認標題包含：{PROFESSOR_COL}, {COURSE_COL}, {RATIO_COL}"
                )
    ...
    try:
        ...
        if target.empty:
            return f"\n>> 查無『{professor_name}』教授的 A+ 比例資料 (Excel 無紀錄)"
